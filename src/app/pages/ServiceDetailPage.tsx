...
const servicesData: Record<string, { title: string; tag: string; image: string; heroDesc: string; features: string[]; process: { step: string; desc: string; icon: typeof Send }[]; priceHint: string; cta: string }> = {
  "malovani-bytu": { title: "Malování bytů a domů", tag: "Rezidenční", image: "/YuSeW8kAj7srJw931dYFVht6BY.avif", heroDesc: "Kompletní malířské služby pro byty a rodinné domy. Od jednopokojových bytů po luxusní rezidence – každý prostor si zaslouží precizní péči.", features: ["Kompletní zakrytí nábytku a podlah fóliemi", "Příprava povrchů — tmelení, broušení, penetrace", "Malování stěn a stropů prémiovými barvami", "Dekorativní lemy, bordury a barevné kombinace", "Finální kontrola kvality a úklid", "Poradenství při výběru barev zdarma"], process: [{ step: "Prohlídka", desc: "Bezplatná návštěva a posouzení stavu", icon: Send }, { step: "Kalkulace", desc: "Detailní cenová nabídka do 24 hodin", icon: Calculator }, { step: "Realizace", desc: "Precizní provedení v dohodnutém termínu", icon: Paintbrush }, { step: "Předání", desc: "Kontrola, úklid a spokojenost", icon: CheckCircle2 }], priceHint: "Od 85 Kč/m²", cta: "Poptat malování bytu" },
  "malovani-pred-prodejem": { title: "Malování před prodejem nebo pronájmem", tag: "Osobní", image: "https://cdn.builder.io/api/v1/image/assets%2Fac4f22b6755541c6871d8f6adda59355%2F6f785ac818cd4504aa3ddbdcc553358c", heroDesc: "Rychlé a efektivní malování, které připraví váš byt či dům na prodej nebo nový pronájem. Vytvoříme neutrální prostředí a pomůžeme nemovitost lépe prezentovat.", features: ["Přemalování výrazných barev do neutrálních odstínů", "Příprava zdí pro čistý a rovnoměrný výsledek", "Opravy a zatmelení drobných nedostatků", "Svěží prostředí připravené na prohlídky", "Rychlá realizace — obvykle 1–2 dny", "Lepší prezentace nemovitosti"], process: [{ step: "Obhlídka", desc: "Posouzení stavu a potřeb", icon: Send }, { step: "Strategie", desc: "Doporučení barev a časový plán", icon: Calculator }, { step: "Realizace", desc: "Rychlé a efektivní provedení", icon: Paintbrush }, { step: "Kontrola", desc: "Finální příprava na prodej/pronájem", icon: CheckCircle2 }], priceHint: "Od 80 Kč/m²", cta: "Poptat malování na prodej" },
  "malovani-kancelari": { title: "Malování kanceláří", tag: "Komerční", image: "/optimized-safe/office.webp", heroDesc: "Profesionální malování komerčních prostor s minimálním narušením provozu. Pracujeme také o víkendech a po dohodě mimo běžnou pracovní dobu.", features: ["Realizace mimo pracovní dobu", "Plán s minimálním omezením provozu", "Antigraffiti a odolné nátěry", "Korporátní barvy podle CI", "Výškové práce podle podmínek objektu", "Pojištění odpovědnosti do 5 mil. Kč"], process: [{ step: "Konzultace", desc: "Posouzení prostor a požadavků", icon: Send }, { step: "Plán", desc: "Harmonogram s ohledem na provoz", icon: Calculator }, { step: "Realizace", desc: "Efektivní provedení", icon: Paintbrush }, { step: "Kontrola", desc: "Inspekce a dokumentace", icon: CheckCircle2 }], priceHint: "Od 75 Kč/m²", cta: "Poptat malování kanceláří" },
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.1 }} className="flex flex-col gap-8">
                ...
                <div className="flex items-start gap-5">
                  ...
                  <p className="font-sans text-foreground m-0" style={{ fontSize: "18px", lineHeight: 1.4, color: "#3d3d47" }}>
                    {service.tag === "Rezidenční" ? "Při práci v obydleném interiéru chráníme vybavení, držíme se domluveného postupu a věnujeme pozornost detailům i závěrečnému úklidu." : "Postup a harmonogram přizpůsobujeme prostoru tak, aby realizace co nejméně zasáhla do běžného provozu."}
                  </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <p className="font-sans text-foreground m-0" style={{ fontSize: "16px", fontWeight: 400, color: "#6b7785" }}>
                    {service.tag === "Rezidenční" ? "Náš přístup k rezidenčním zakázkám" : "Náš přístup ke komerčním zakázkám"}
                  </p>
                </div>
              </motion.div>
            </div>
